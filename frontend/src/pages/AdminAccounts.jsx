    ...
    <PortalLayout>
      <div className="space-y-4">
        <div className="flex items-center justify-between flex-wrap gap-2">
          <h2 className="text-lg font-semibold text-[#0E1B2C]">Accounts</h2>
          <div className="flex gap-2">
            <select value={year} onChange={e => setYear(e.target.value)} className="border border-[#E2D8C2] rounded-lg px-2 py-1.5 text-xs">
        ...
        {/* Tabs */}
        <div className="flex gap-2 border-b border-[#E2D8C2] pb-1">
          {[["overview", "Overview"], ["transactions", "Transactions"], ["balance", "Balance Sheet"]].map(([k, l]) => (
            <button key={k} onClick={() => setTab(k)}
              className={`px-3 py-1.5 text-xs font-medium rounded-t-lg ${tab === k ? "bg-[#024396] text-white" : "text-[#5C677D] hover:bg-[#F5F1EB]"}`}>
              {l}
            </button>
          ))}
        </div>

        {loading ? <p className="text-sm text-[#5C677D]">Loading...</p> : (
          <>
            {/* Overview Tab */}
            {tab === "overview" && summary && (
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  <div className="bg-white rounded-xl border border-[#E2D8C2] p-4">
                    <h4 className="text-xs font-semibold text-[#5C677D] mb-2 uppercase tracking-wider">Assets</h4>
                    <p className="text-lg font-semibold text-blue-600">{fmtINR(summary.total_assets)}</p>
                  </div>
                  <div className="bg-white rounded-xl border border-[#E2D8C2] p-4">
                    <h4 className="text-xs font-semibold text-[#5C677D] mb-2 uppercase tracking-wider">Liabilities</h4>
                ...
                {summary.category_breakdown && Object.keys(summary.category_breakdown).length > 0 && (
                  <div className="bg-white rounded-xl border border-[#E2D8C2] p-4">
                    <h4 className="text-xs font-semibold text-[#5C677D] mb-3 uppercase tracking-wider">Category Breakdown</h4>
                    <div className="space-y-3">
                      {Object.entries(summary.category_breakdown).map(([type, cats]) => (
                        <div key={type}>
                          <p className="text-xs font-semibold text-[#0E1B2C] capitalize mb-1">{type}</p>
                          <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
                            {Object.entries(cats).map(([cat, amt]) => (
                              <div key={cat} className="text-xs text-[#2A364B] bg-[#F5F1EB] rounded-lg px-2 py-1.5">
                ...
                {summary.monthly && Object.keys(summary.monthly).length > 0 && (
                  <div className="bg-white rounded-xl border border-[#E2D8C2] p-4">
                    <h4 className="text-xs font-semibold text-[#5C677D] mb-3 uppercase tracking-wider">Monthly P&L</h4>
                    <div className="space-y-1.5">
                      {Object.entries(summary.monthly).sort().map(([m, d]) => (
                        <div key={m} className="flex justify-between text-xs text-[#2A364B] bg-[#F5F1EB] rounded-lg px-3 py-2">
            ...
            {tab === "transactions" && (
              <div className="space-y-3">
                <div className="flex gap-2 flex-wrap">
                  <button onClick={() => setFilterType("")} className={`px-3 py-1 rounded-lg text-xs border ${!filterType ? "bg-[#024396] text-white border-[#024396]" : "bg-white border-[#E2D8C2]"}`}>All</button>
                  {TYPES.map(t => (
                    <button key={t} onClick={() => setFilterType(t)} className={`px-3 py-1 rounded-lg text-xs border capitalize ${filterType === t ? "bg-[#024396] text-white border-[#024396]" : "bg-white border-[#E2D8C2]"}`}>{t}</button>
                  ))}
                </div>
                {transactions.length === 0 ? (
                  <p className="text-sm text-[#5C677D] text-center py-8">No transactions found</p>
                ) : (
                  <div className="space-y-2">
                    {transactions.map(txn => (
                      <div key={txn.id} className="bg-white rounded-xl border border-[#E2D8C2] p-3 flex items-center gap-3">
                        <div className={`px-2 py-1 rounded-lg text-[10px] font-semibold capitalize ${typeColor[txn.type] || ""}`}>{txn.type}</div>
                        <div className="flex-1 min-w-0">
                          <p className="text-sm font-medium text-[#0E1B2C] truncate">{txn.category}{txn.description ? ` — ${txn.description}` : ""}</p>
                          <p className="text-[10px] text-[#5C677D]">{txn.date} {txn.payment_mode && `· ${txn.payment_mode}`} {txn.reference && `· Ref: ${txn.reference}`}</p>
                        </div>
            ...
            {/* Balance Sheet Tab */}
            {tab === "balance" && summary && (
              <div className="bg-white rounded-xl border border-[#E2D8C2] p-5">
                <h3 className="text-center font-semibold text-[#0E1B2C] mb-1">Balance Sheet — The Financial Doctor</h3>
                <p className="text-center text-xs text-[#5C677D] mb-4">As on 31st December {year}</p>
                <div className="grid grid-cols-2 gap-6">
                  <div>
                    <h4 className="text-xs font-bold text-[#024396] uppercase tracking-wider border-b border-[#024396] pb-1 mb-2">Assets</h4>
                    {summary.category_breakdown?.asset && Object.entries(summary.category_breakdown.asset).map(([k, v]) => (
                      <div key={k} className="flex justify-between text-xs text-[#2A364B] py-1 border-b border-dotted border-[#E2D8C2]">
                        <span>{k}</span><span className="font-medium">{fmtINR(v)}</span>
                      </div>
                    ))}
                    <div className="flex justify-between text-sm font-bold text-[#0E1B2C] pt-2 border-t-2 border-[#0E1B2C] mt-2">
                ...
                <div className="mt-4 pt-3 border-t-2 border-[#024396] flex justify-between">
                  <span className="text-sm font-bold text-[#024396]">Company Net Worth</span>
                  <span className="text-lg font-bold text-[#024396]">{fmtINR(summary.networth)}</span>
                </div>
                <div className="mt-3 pt-3 border-t border-[#E2D8C2]">
                  <div className="flex justify-between text-xs text-[#2A364B] py-0.5"><span>Total Revenue ({year})</span><span className="text-green-600 font-medium">{fmtINR(summary.total_income)}</span></div>
                  <div className="flex justify-between text-xs text-[#2A364B] py-0.5"><span>Total Expenses ({year})</span><span className="text-red-600 font-medium">{fmtINR(summary.total_expense)}</span></div>
                  <div className="flex justify-between text-sm font-bold text-[#0E1B2C] pt-1 border-t border-[#E2D8C2] mt-1">
        ...
        {showAdd && (
          <div className="fixed inset-0 z-[100] bg-black/50 flex items-center justify-center p-4" onClick={() => setShowAdd(false)}>
            <form onSubmit={handleSubmit} className="bg-white rounded-2xl p-6 max-w-md w-full space-y-3 max-h-[90vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
              <h3 className="text-lg font-semibold text-[#0E1B2C]">Add Entry</h3>

              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="text-xs text-[#5C677D] block mb-1">Date</label>
                  <input type="date" value={form.date} onChange={e => setForm(f => ({ ...f, date: e.target.value }))}
                    className="w-full border border-[#E2D8C2] rounded-lg px-3 py-2 text-sm" />
                </div>
                <div>
                  <label className="text-xs text-[#5C677D] block mb-1">Type</label>
                  <select value={form.type} onChange={e => setForm(f => ({ ...f, type: e.target.value, category: (CATEGORIES[e.target.value] || [])[0] || "" }))}
                    className="w-full border border-[#E2D8C2] rounded-lg px-3 py-2 text-sm capitalize">
              ...
              <div>
                <label className="text-xs text-[#5C677D] block mb-1">Amount (₹)</label>
                <input type="number" value={form.amount} onChange={e => setForm(f => ({ ...f, amount: e.target.value }))}
                  className="w-full border border-[#E2D8C2] rounded-lg px-3 py-2 text-sm" placeholder="Enter amount" />
              </div>

              <div>
                <label className="text-xs text-[#5C677D] block mb-1">Description</label>
                <input value={form.description} onChange={e => setForm(f => ({ ...f, description: e.target.value }))}
                  className="w-full border border-[#E2D8C2] rounded-lg px-3 py-2 text-sm" placeholder="Optional details" />
              </div>

              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="text-xs text-[#5C677D] block mb-1">Payment Mode</label>
                ...
                <div>
                  <label className="text-xs text-[#5C677D] block mb-1">Reference</label>
                  <input value={form.reference} onChange={e => setForm(f => ({ ...f, reference: e.target.value }))}
                    className="w-full border border-[#E2D8C2] rounded-lg px-3 py-2 text-sm" placeholder="Bill/Txn no." />
                </div>
              </div>
