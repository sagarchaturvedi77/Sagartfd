  ...
  return (
    <PortalLayout>
      <div className="space-y-8">
        <div>
          <h2 className="text-xl font-serif text-[#0E1B2C]">Website Content Control</h2>
          <p className="text-xs text-[#2A364B]/50">Manage website popups, top banner/marquee from here</p>
        </div>

        {/* Top Banner / Marquee */}
        <div className="bg-white rounded-2xl border border-[#E2D8C2] shadow-sm p-6 space-y-4">
          <div className="flex items-center justify-between">
            <h3 className="text-sm font-semibold text-[#0E1B2C]">Top Banner / Marquee</h3>
            <label className="flex items-center gap-2 cursor-pointer">
              ...
              <span className="text-xs text-[#2A364B]/60">{banner.enabled ? "Active" : "Disabled"}</span>
            </label>
          </div>
          <div className="space-y-3">
            <div>
              <label className="block text-xs text-[#2A364B]/60 mb-1">Banner Text (marquee)</label>
            ...
            <div>
              <label className="block text-xs text-[#2A364B]/60 mb-1">Link (optional, click to open)</label>
          ...
          {banner.enabled && banner.text && (
            <div className="bg-[#024396] text-white text-xs py-2 px-4 rounded-xl overflow-hidden">
              <div className="animate-marquee whitespace-nowrap">{banner.text}</div>
            </div>
          )}
          <button onClick={saveBanner} disabled={saving}
            className="px-5 py-2.5 rounded-xl text-sm font-semibold text-white bg-gradient-to-r from-[#024396] to-[#0356c4] disabled:opacity-50">
        ...
        <div className="bg-white rounded-2xl border border-[#E2D8C2] shadow-sm p-6 space-y-4">
          <div className="flex items-center justify-between">
            <h3 className="text-sm font-semibold text-[#0E1B2C]">Website Popup</h3>
            <label className="flex items-center gap-2 cursor-pointer">
              <input type="checkbox" checked={popup.enabled} onChange={e => setPopup({...popup, enabled: e.target.checked})}
                className="rounded border-[#E2D8C2]" />
              <span className="text-xs text-[#2A364B]/60">{popup.enabled ? "Active" : "Disabled"}</span>
            </label>
          </div>
          <div className="space-y-3">
            <div>
              <label className="block text-xs text-[#2A364B]/60 mb-1">Popup Title</label>
              <input value={popup.title} onChange={e => setPopup({...popup, title: e.target.value})} className={field} placeholder="e.g. Welcome to TFD!" />
            </div>
            <div>
              <label className="block text-xs text-[#2A364B]/60 mb-1">Popup Body / Message</label>
            ...
            <div>
              <label className="block text-xs text-[#2A364B]/60 mb-1">Link / CTA URL (optional)</label>
          ...
          {popup.enabled && popup.title && (
            <div className="bg-[#FBF7EE] rounded-xl p-4 border border-[#E2D8C2]">
              <p className="text-xs text-[#2A364B]/40 mb-1">Preview:</p>
              <p className="text-sm font-semibold text-[#0E1B2C]">{popup.title}</p>
              <p className="text-xs text-[#2A364B]/70 mt-1">{popup.body}</p>
              {popup.link && <p className="text-xs text-[#024396] mt-1">{popup.link}</p>}
            </div>
          )}
